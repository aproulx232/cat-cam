...
def annotate_objects(image, results, labels):
    ...
    for obj in results:
        ...
        draw.text([xmin, ymin],
                  '%s\n%.2f' % (labels[obj['class_id']], obj['score']),  fill='Red')
            ...
            try:
                while True:
                    with output.condition:
                        ...
                        image = Image.open(output.buffer).convert('RGB').resize(
                            (input_width, input_height), Image.ANTIALIAS)
                        start_time = time.monotonic()

                        results = detect_objects(
                            interpreter, image, args.threshold)
...
# sudo python3 stream_object_detection.py --model /tmp/detect.tflite --labels /tmp/coco_labels.txt
